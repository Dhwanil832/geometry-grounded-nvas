...
Training infrastructure adapted from:
...
def main_per_worker(rank, world_size, *args):
    ...
    if args.distributed:
        ...
        model = getattr(impm(cfg.model.file), 'build_model')(cfg, train_dataset.scene)
        model.to(device)
...
# ---- build eval loader (only if eval_dataset is non-empty) ----
    eval_loader = None
    if eval_dataset is not None and len(eval_dataset) > 0:
        ...
        print("[WARN] eval_dataset is empty. Skipping eval_loader.")

...
Evaluation script adapted from:
...
model = getattr(impm(cfg.model.file), 'build_model')(cfg, test_dataset.scene)
...
with torch.no_grad():
    for ori_val_data in tqdm(test_loader):
        ...
        # ---- RT60 metric (DISABLED if estimator missing) ----
        # rt60_estimator = load_rt60_estimator(device)
        rt60_estimator = None

        if rt60_estimator is not None:
            pred_rt60 = estimate_rt60(rt60_estimator, pred_wav.reshape(-1, pred_wav.shape[-1]))
            tgt_rt60 = estimate_rt60(
                rt60_estimator,
                tgt_wav[..., :pred_wav.shape[-1]].reshape(-1, pred_wav.shape[-1])
            )
            stats['rte'].append((pred_rt60 - tgt_rt60).abs().mean().item())
        # else: skip RT60 entirely
        # -----------------------------------------------
        ...
        stats['mag'].append(
            (
                (pred_spec_l - tgt_spec_l).pow(2).sqrt().mean((1, 2))
...
with open(f'av_results/{cfg.output_dir}_{split}.pkl', 'wb') as file:

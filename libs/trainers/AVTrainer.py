...
class Trainer(object):
    ...
    def _forward(self, data):
        index, target_pose, vggt_tokens, context_poses, gt_waveform, source_gt_audio = data
        pred_wav = self.model(target_pose=target_pose, vggt_tokens=vggt_tokens, context_poses=context_poses, source_gt_audio=source_gt_audio)
    ...
    def train(self, train_loader, eval_loader, save_dir=None):
        ...
        for data in metric_logger.log_every(train_loader, print_freq, header, self.logger):
            ...
            if not math.isfinite(sum(p.sum() for p in self.model.module.parameters() if p.requires_grad)):
                self.logger.info("NaN/Inf detected in model parameters after step — stopping training")
                sys.exit(1)
    ...
    def quick_val(self, eval_data_iter):
        ...
        with torch.no_grad():
            ...
            index, target_pose, vggt_tokens, context_poses, gt_waveform, source_gt_audio = val_data
            tar_out = gt_waveform.flatten(0, 1)
            pred_wav = self.model(target_pose=target_pose, vggt_tokens=vggt_tokens, context_poses=context_poses, source_gt_audio=source_gt_audio, is_val=True)
            B = len(tar_out)
            ...
            self.evaluator.evaluate(np_pred_ir, np_gt_ir, sr=self.cfg.dataset.sr)
